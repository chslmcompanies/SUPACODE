...
const SignalTable: React.FC<SignalTableProps> = ({ projects, onProjectSelect }) => {
  ...
  // All unique values per column for the filter dropdown
  ...
  const toggleFilterValue = useCallback((col: string, val: string) => {
    setColFilters(prev => {
      ...
      // If currently "show all" (empty set), clicking a value means "exclude that one"
      if (cur.size === 0) {
      ...
      // If all selected again, collapse back to empty (= show all)
  ...
  const clearFilter = useCallback((col: string) => {
    setColFilters(prev => ({ ...prev, [col]: new Set(['__none__']) })); // show nothing
  }, []);
